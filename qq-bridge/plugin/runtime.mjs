...
/**
 * 插件主体（由薄壳 index.mjs 动态加载）。
 * @param {object} ctx        Cordis context
 * @param {object} rawConfig  插件配置
 * @param {number} ts         加载时间戳（薄壳传入），用于给所有业务子模块
 *                            追加 query 以绕过 ESM 缓存 → 改代码热更新
 */
export function apply(ctx, rawConfig = {}, ts = Date.now()) {
  // 动态加载全部业务子模块（同一 ts，保证整树一致刷新）
  const load = (u) => import(`./${u}?t=${ts}`)
  const B = {}
  return Promise.all([
    load('onebot-client.mjs'),
    load('config.mjs'),
    load('persona.mjs'),
    load('group-config.mjs'),
    load('energy.mjs'),
    load('session.mjs'),
    load('reply-buffer.mjs'),
    load('handlers.mjs'),
  ]).then(([onebotMod, configMod, personaMod, groupMod, energyMod, sessionMod, replyMod, handlerMod]) => {
    Object.assign(B, { onebotMod, configMod, personaMod, groupMod, energyMod, sessionMod, replyMod, handlerMod })
    return run(ctx, rawConfig, B)
  })
}

/** 真正的业务逻辑（apply 动态加载完模块后调用）。 */
function run(ctx, rawConfig, B) {
  const { OneBotClient } = B.onebotMod
  const { resolveConfig } = B.configMod
  const { buildPersonaPrompt, ackText } = B.personaMod
  const { createGroupManager } = B.groupMod
  const { createEnergyManager } = B.energyMod
  const { createSessionManager } = B.sessionMod
  const { createReplyBuffer } = B.replyMod
  const { createHandlers } = B.handlerMod
